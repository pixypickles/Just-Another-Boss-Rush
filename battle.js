...
function updateUI(){
 const alive=heroes.filter(x=>!x.dead),h=heroes[heroIndex],dBtn=document.getElementById('d');if(dBtn){const soloReady=alive.length===1;dBtn.innerHTML=soloReady?'<b>D</b>専用技':'<b>D</b>交代';dBtn.classList.toggle('soloReady',soloReady);}ui.stageNo.textContent=trainingMode?'TRAINING':foxMode?'FOX BATTLE':bushinMode?'SPECIAL BOSS':'BOSS '+(bossIndex+1)+' / 5';ui.bossName.textContent=trainingMode?(trainingChallenge==='limit'?`60秒撃破　${trainingKills}体　残り ${Math.max(0,60-trainingElapsed).toFixed(1)}秒`:`100体撃破　${trainingKills} / 100　${formatTrainingTime(trainingElapsed)}`):foxMode?'白狐忍・三影':bushinMode?'武神':bossDefs[bossIndex].name;ui.bossFill.style.width=trainingMode?(trainingChallenge==='limit'?Math.min(100,trainingElapsed/60*100):trainingKills)+'%':(boss.hp/boss.maxHp*100)+'%';ui.heroName.textContent=heroInfo[h.type].name;ui.ability.textContent=heroInfo[h.type].ability;ui.hpFill.style.width=(h.hp/h.maxHp*100)+'%';ui.hpText.textContent=Math.ceil(h.hp)+' / '+h.maxHp+' HP';
