...
function updateShots(dt){for(let i=shots.length-1;i>=0;i--){const s=shots[i];s.life-=dt;if(s.spiral){s.spiralAngle+=s.spiralAngular*dt;s.spiralRadius+=s.spiralRadial*dt;const c=s.spiralOwner&&!s.spiralOwner.dead?s.spiralOwner:null,cx=c?c.x:s.spiralX,cy=c?c.y:s.spiralY;s.x=cx+Math.cos(s.spiralAngle)*s.spiralRadius;s.y=cy+Math.sin(s.spiralAngle)*s.spiralRadius;s.vx=-Math.sin(s.spiralAngle)*s.spiralAngular*s.spiralRadius+Math.cos(s.spiralAngle)*s.spiralRadial;s.vy=Math.cos(s.spiralAngle)*s.spiralAngular*s.spiralRadius+Math.sin(s.spiralAngle)*s.spiralRadial}else{if(s.homing&&!boss.dead){const desired=norm(boss.x-s.x,boss.y-s.y),speed=Math.hypot(s.vx,s.vy),cur=norm(s.vx,s.vy),blend=Math.min(1,dt*4.6),nn=norm(cur.x*(1-blend)+desired.x*blend,cur.y*(1-blend)+desired.y*blend);s.vx=nn.x*speed;s.vy=nn.y*speed}s.x+=s.vx*dt;s.y+=s.vy*dt;}if(s.life<=0||s.x<45||s.x>955||s.y<110||s.y>940){shots.splice(i,1);continue}if(s.team==='hero'){
...
function updateMinions(dt){
 for(let i=minions.length-1;i>=0;i--){
  ...
  if(m.life<=0||m.hp<=0){if(m.hp<=0){burst(m.x,m.y,'#b46cff',15,210);if(trainingMode&&m.training){trainingKills++;if(trainingChallenge==='time'&&trainingKills>=100){minions.splice(i,1);finishTraining();return}}}minions.splice(i,1);continue}
...
function updateUI(){
 const alive=heroes.filter(x=>!x.dead),h=heroes[heroIndex],dBtn=document.getElementById('d');if(trainingMode&&trainingChallenge==='mole'){const labels={a:'左叩き',b:'上叩き',c:'下叩き',d:'右叩き'};for(const id of ['a','b','c','d']){const el=document.getElementById(id);if(el)el.innerHTML=`<b>${id.toUpperCase()}</b>${labels[id]}`}}else if(trainingMode&&(trainingChallenge==='shooting'||trainingChallenge==='precision')){const labels={a:'射撃',b:'使用不可',c:'使用不可',d:'使用不可'};for(const id of ['a','b','c','d']){const el=document.getElementById(id);if(el)el.innerHTML=`<b>${id.toUpperCase()}</b>${labels[id]}`}}if(dBtn&&!(trainingMode&&(trainingChallenge==='mole'||trainingChallenge==='shooting'||trainingChallenge==='precision'))){const soloReady=alive.length===1;dBtn.innerHTML=soloReady?'<b>D</b>専用技':'<b>D</b>交代';dBtn.classList.toggle('soloReady',soloReady);}ui.stageNo.textContent=trainingMode?(trainingChallenge==='mole'?'MOLE TRAINING':trainingChallenge==='shooting'?'INTERCEPT TRAINING':trainingChallenge==='precision'?'PRECISION SHOOTING':'TRAINING'):foxMode?'FOX BATTLE':bushinMode?'SPECIAL BOSS':'BOSS '+(bossIndex+1)+' / 5';ui.bossName.textContent=trainingMode?(trainingChallenge==='mole'?`モグラ叩き　${moleScore}点　残り ${Math.max(0,MOLE_DURATION-trainingElapsed).toFixed(1)}秒`:trainingChallenge==='shooting'?`迎撃訓練　${shootingScore}点　残り ${Math.max(0,SHOOTING_DURATION-trainingElapsed).toFixed(1)}秒`:trainingChallenge==='precision'?`射撃訓練　${precisionScore}点　残り ${Math.max(0,PRECISION_DURATION-trainingElapsed).toFixed(1)}秒`:trainingChallenge==='limit'?`60秒撃破　${trainingKills}体　残り ${Math.max(0,60-trainingElapsed).toFixed(1)}秒`:`100体撃破　${trainingKills} / 100　${formatTrainingTime(trainingElapsed)}`):foxMode?'白狐忍・三影':bushinMode?'武神':bossDefs[bossIndex].name;ui.bossFill.style.width=trainingMode?(trainingChallenge==='mole'?Math.min(100,trainingElapsed/MOLE_DURATION*100):trainingChallenge==='shooting'?Math.min(100,trainingElapsed/SHOOTING_DURATION*100):trainingChallenge==='precision'?Math.min(100,trainingElapsed/PRECISION_DURATION*100):trainingChallenge==='limit'?Math.min(100,trainingElapsed/60*100):trainingKills)+'%':(boss.hp/boss.maxHp*100)+'%';ui.heroName.textContent=heroInfo[h.type].name;ui.ability.textContent=heroInfo[h.type].ability;ui.hpFill.style.width=(h.hp/h.maxHp*100)+'%';ui.hpText.textContent=Math.ceil(h.hp)+' / '+h.maxHp+' HP';
...
function update(dt){bloodBeams.length=0;if(trainingMode&&trainingChallenge==='mole'){updateMoleGame(dt);return}if(trainingMode&&trainingChallenge==='shooting'){updateShootingGame(dt);return}if(trainingMode&&trainingChallenge==='precision'){updatePrecisionShootingGame(dt);return}if(timeStop>0){timeStop=Math.max(0,timeStop-dt);const alive=heroes.filter(h=>!h.dead);if(alive.length&&heroes[heroIndex]?.dead)heroIndex=heroes.indexOf(alive[0]);if(pressed.has('KeyI')){pressed.delete('KeyI');if(alive.length===1)alive[0].safeUse('soloSkill');else nextHero()}heroes.forEach((h,i)=>h.update(dt,i));updateUI();released.clear();return}if(trainingMode&&!trainingFinished){trainingElapsed+=dt;if(trainingChallenge==='limit'&&trainingElapsed>=60){trainingElapsed=60;finishTraining();return}trainingSpawnTimer-=dt;for(let i=0;i<trainingSpawnFlash.length;i++)trainingSpawnFlash[i]=Math.max(0,trainingSpawnFlash[i]-dt);while(trainingSpawnTimer<=0&&minions.length<trainingMinionLimit()&&(trainingChallenge==='limit'||trainingKills+minions.length<100)){spawnTrainingMinion();trainingSpawnTimer+=.24}}const aliveNow=heroes.filter(h=>!h.dead);if(aliveNow.length&&heroes[heroIndex]?.dead)heroIndex=heroes.indexOf(aliveNow[0]);if(pressed.has('KeyI')){pressed.delete('KeyI');if(awakenedMode&&aliveNow.length===1)dInputBuffer=.14;else if(aliveNow.length===1)aliveNow[0].safeUse('soloSkill');else nextHero()}if(awakenedMode&&aliveNow.length===1&&dInputBuffer>0){dInputBuffer=Math.max(0,dInputBuffer-dt);const h=aliveNow[0];if(h&&!h.dead&&h.soloCd<=0){h.safeUse('soloSkill');dInputBuffer=0}}else if(!awakenedMode)dInputBuffer=0;if(heroes.every(h=>h.dead)){running=false;notice('PARTY DOWN','#ff7a86',1500);setTimeout(()=>ui.start.style.display='grid',1000);return}if(transition>0){transition-=dt;if(transition<=0){bossIndex++;if(bossIndex>=5){running=false;const solo=selectedTypes.length===1,soloType=solo?selectedTypes[0]:null,unlocks=[];const cursedTrio=selectedTypes.length===3&&['dracula','dragonknight','mimic'].every(t=>selectedTypes.includes(t));if(!awakenedMode&&!bushinMode&&!foxMode&&!isTrainingUnlocked()){saveTrainingUnlock();if(window.refreshTrainingMenu)window.refreshTrainingMenu();unlocks.push('修行場・100体撃破タイムアタックが解放された！')}if(((!awakenedMode&&solo)||cursedTrio)&&!isBushinUnlocked()){saveBushinUnlock();addBushinOptions();unlocks.push('武神挑戦モードが解放された！')}if(!isAwakeningUnlocked()){saveAwakeningUnlock();addAwakeningOption();unlocks.push('覚醒無双モードが解放された！')}if(!isMonkUnlocked()){saveMonkUnlock();unlockMonkChoice();unlocks.push('モンクが仲間になった！')}if(!isDragonKnightUnlocked()){saveDragonKnightUnlock();unlockDragonKnightChoice();unlocks.push('魔王による竜の呪いが解け、竜騎士が仲間になった！')}if(soloType==='healer'&&!isHighPriestUnlocked()){saveHighPriestUnlock();unlockHighPriestChoice();unlocks.push('聖なる力を極め、ハイプリーストが仲間になった！')}if(soloType==='knight'&&!isMagicbladeUnlocked()){saveMagicbladeUnlock();unlockMagicbladeChoice();unlocks.push('闇の剣技を継承し、魔剣士が仲間になった！')}if(soloType==='mage'&&!isRunemageUnlocked()){saveRunemageUnlock();unlockRunemageChoice();unlocks.push('古代ルーンを読み解き、ルーンメイジが仲間になった！')}if(soloType==='monk'&&!isQigongUnlocked()){saveQigongUnlock();unlockQigongChoice();unlocks.push('気の極意を悟り、気功師が仲間になった！')}if(!awakenedMode&&selectedTypes.length<=2&&!isNinjaUnlocked()){saveNinjaUnlock();unlockNinjaChoice();unlocks.push('少数精鋭の戦いを見届け、忍者が仲間になった！')}if(soloType==='dragonknight'&&!isMimicUnlocked()){saveMimicUnlock();unlockMimicChoice();unlocks.push('魔王によるケルベロスの呪いが解け、模倣術師が仲間になった！')}if(!awakenedMode&&soloType==='ninja'&&!isFoxModeUnlocked()){saveFoxModeUnlock();addFoxModeOption();unlocks.push('忍びの極意を示し、キツネ対戦モードが解放された！')}if(difficultyMode==='hard3'&&!isHard3Cleared()){saveHard3Clear();unlocks.push('3人ハードモードをクリアした！')}if(difficultyMode==='hard2'&&!isHard2Cleared()){saveHard2Clear();unlocks.push('2人ハードモードをクリアした！')}if(difficultyMode==='hard1'&&!isHard1Cleared()){saveHard1Clear();unlocks.push('1人ハードモードをクリアした！')}if(difficultyMode==='oni3'&&!isOniHard3Cleared()){saveOniHard3Clear();unlocks.push('3人鬼ハードモードをクリアした！')}document.getElementById('loadStatus').textContent=unlocks.length?unlocks.join(' / '):'クリア済み追加要素は解放済みです。';notice(unlocks.length?unlocks.join(' / '):'ALL CHAMBERS CLEARED!','#fff08a',4200);setTimeout(()=>ui.start.style.display='grid',1800);bossIndex=0}else setupBattle()}return}heroes.forEach((h,i)=>h.update(dt,i));boss.update(dt);updateRunes(dt);updateShots(dt);updateMinions(dt);const dominator=heroes.find(h=>!h.dead&&h.type==='dracula'&&h.dominationTime>0);if(dominator&&boss&&!boss.dead){for(let i=minions.length-1;i>=0;i--){const m=minions[i];dominator.summonBats(1,m.x,m.y,42);burst(m.x,m.y,'#8f244c',12,160);if(trainingMode&&m.training)trainingKills++;minions.splice(i,1);if(trainingMode&&trainingChallenge==='time'&&trainingKills>=100){finishTraining();return}}}enforceQigongWalls();updateLasers(dt);for(let i=walls.length-1;i>=0;i--){walls[i].life-=dt;if(walls[i].life<=0)walls.splice(i,1)}for(let i=slashes.length-1;i>=0;i--){slashes[i].life-=dt;if(slashes[i].life<=0)slashes.splice(i,1)}for(let i=fistTrails.length-1;i>=0;i--){const f=fistTrails[i];f.life-=dt;f.x+=f.dx*90*dt;f.y+=f.dy*90*dt;if(f.life<=0)fistTrails.splice(i,1)}for(let i=particles.length-1;i>=0;i--){const p=particles[i];p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vx*=.96;p.vy*=.96;if(p.life<=0)particles.splice(i,1)}for(let i=holyFx.length-1;i>=0;i--){holyFx[i].life-=dt;if(holyFx[i].life<=0)holyFx.splice(i,1)}for(let i=holyDots.length-1;i>=0;i--){const d=holyDots[i];d.life-=dt;d.tick+=dt;if(!d.target||d.target.dead||d.target.hp<=0||d.life<=0){holyDots.splice(i,1);continue}while(d.tick>=.25){d.tick-=.25;if(d.target===boss)d.target.hurt(d.rate*.25);else d.target.hp-=d.rate*.25}}updateUI();released.clear()}
