...
function updateUI(){
 const alive=heroes.filter(x=>!x.dead),h=heroes[heroIndex],dBtn=document.getElementById('d');if(trainingMode&&trainingChallenge==='mole'){const labels={a:'左叩き',b:'上叩き',c:'下叩き',d:'右叩き'};for(const id of ['a','b','c','d']){const el=document.getElementById(id);if(el)el.innerHTML=`<b>${id.toUpperCase()}</b>${labels[id]}`}}else if(trainingMode&&trainingChallenge==='shooting'){const labels={a:'射撃',b:'使用不可',c:'使用不可',d:'使用不可'};for(const id of ['a','b','c','d']){const el=document.getElementById(id);if(el)el.innerHTML=`<b>${id.toUpperCase()}</b>${labels[id]}`}}if(dBtn&&!(trainingMode&&(trainingChallenge==='mole'||trainingChallenge==='shooting'))){const soloReady=alive.length===1;dBtn.innerHTML=soloReady?'<b>D</b>専用技':'<b>D</b>交代';dBtn.classList.toggle('soloReady',soloReady);}ui.stageNo.textContent=trainingMode?(trainingChallenge==='mole'?'MOLE TRAINING':trainingChallenge==='shooting'?'SHOOTING TRAINING':'TRAINING'):foxMode?'FOX BATTLE':bushinMode?'SPECIAL BOSS':'BOSS '+(bossIndex+1)+' / 5';ui.bossName.textContent=trainingMode?(trainingChallenge==='mole'?`モグラ叩き　${moleScore}点　残り ${Math.max(0,MOLE_DURATION-trainingElapsed).toFixed(1)}秒`:trainingChallenge==='shooting'?`射撃訓練　${shootingScore}点　残り ${Math.max(0,SHOOTING_DURATION-trainingElapsed).toFixed(1)}秒`:trainingChallenge==='limit'?`60秒撃破　${trainingKills}体　残り ${Math.max(0,60-trainingElapsed).toFixed(1)}秒`:`100体撃破　${trainingKills} / 100　${formatTrainingTime(trainingElapsed)}`):foxMode?'白狐忍・三影':bushinMode?'武神':bossDefs[bossIndex].name;ui.bossFill.style.width=trainingMode?(trainingChallenge==='mole'?Math.min(100,trainingElapsed/MOLE_DURATION*100):trainingChallenge==='shooting'?Math.min(100,trainingElapsed/SHOOTING_DURATION*100):trainingChallenge==='limit'?Math.min(100,trainingElapsed/60*100):trainingKills)+'%':(boss.hp/boss.maxHp*100)+'%';ui.heroName.textContent=heroInfo[h.type].name;ui.ability.textContent=heroInfo[h.type].ability;ui.hpFill.style.width=(h.hp/h.maxHp*100)+'%';ui.hpText.textContent=Math.ceil(h.hp)+' / '+h.maxHp+' HP';
