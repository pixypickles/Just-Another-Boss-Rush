...
class Boss{
 ...
 updateBushin(dt){
  this.inv=Math.max(0,this.inv-dt);this.touchCd=Math.max(0,this.touchCd-dt);this.bushinTimer-=dt;
  ...
  if(this.bushinState==='punchWindup'&&this.bushinTimer<=0){
   ...
   const t=this.bushinCounterTarget&& !this.bushinCounterTarget.dead?this.bushinCounterTarget:target;
   this.bushinState='counterRise';this.bushinTimer=.26;this.bushinAir=1;this.x=clamp(t.x+(t.x<500?95:-95),100,900);this.y=clamp(t.y+55,160,880);this.bushinStart={x:this.x,y:this.y};this.bushinTarget={x:t.x,y:t.y};
  }else if(this.bushinState==='counterRise'){
   const p=clamp(1-this.bushinTimer/.26,0,1);this.bushinAir=Math.sin(p*Math.PI/2)*165;
   if(this.bushinTimer<=0){const t=this.bushinCounterTarget&& !this.bushinCounterTarget.dead?this.bushinCounterTarget:target;this.bushinState='counterPause';this.bushinTimer=.08;this.bushinAir=165;this.bushinTarget={x:t.x,y:t.y};this.bushinStart={x:this.x,y:this.y}}
  }else if(this.bushinState==='counterPause'&&this.bushinTimer<=0){
   this.bushinState='counterDive';this.bushinTimer=.28;this.bushinKick=true;this.bushinKickDirX=(this.bushinTarget.x-this.bushinStart.x)||1;this.bushinHitDone=false
  }else if(this.bushinState==='counterDive'){
   const p=clamp(1-this.bushinTimer/.28,0,1),e=1-Math.pow(1-p,2);this.x=this.bushinStart.x+(this.bushinTarget.x-this.bushinStart.x)*e;this.y=this.bushinStart.y+(this.bushinTarget.y-this.bushinStart.y)*e;this.bushinAir=165*(1-e);clampArena(this);
   if(!this.bushinHitDone&&p>.48){this.bushinHitDone=true;for(const h of heroes)if(!h.dead&&Math.hypot(h.x-this.x,h.y-this.y)<98+h.r){const k=norm(h.x-this.x,h.y-this.y);h.hurt(this.damage(104),k.x*600,k.y*600);shake=20}}
   if(this.bushinTimer<=0){burst(this.x,this.y,'#8fd8ff',30,420);this.finishBushinAction(1.05)}
  }
 }
 ...
 triggerBushinCounter(){if(this.bushinCounterTriggered)return;this.bushinCounterTriggered=true;this.bushinState='counterVanish';this.bushinTimer=.09;this.bushinAir=0;this.bushinKick=false;this.vx=this.vy=0;this.slow=this.freezeStop=0;burst(this.x,this.y,'#8fd8ff',38,360);notice('見切られた！ 武神の反撃！','#b8e8ff',720)}
