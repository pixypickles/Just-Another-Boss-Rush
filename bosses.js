...
class Boss{
 ...
 summon(count=2,strong=false){
  ...
  for(let i=0;i<spawnCount;i++){const a=Math.PI*2*i/spawnCount+Math.random()*.8,d=105+rnd(0,85);minions.push({
   ...
   hp:Math.round((strong?170:105)*(hardMode?1.7:1)),maxHp:Math.round((strong?170:105)*(hardMode?1.7:1)),damage:this.damage(strong?34:24),cd:rnd(.2,.7),life:strong?22:16,strong
  })}
...
function setupBattle(){
 ...
 updateUI();notice(foxMode?'キツネ対戦――三影を打ち破れ！':bushinMode?'武神挑戦――力を示せ！':(awakenedMode?'覚醒・封印の間 ':hardMode?'ハード・封印の間 ':'封印の間 ')+(bossIndex+1),foxMode?'#bceeff':bushinMode?'#ffc98a':awakenedMode?'#fff08a':hardMode?'#ff9b8f':'#ffd88b',1100)
}
