...
class Boss{
 constructor(def){const partyScale=def.kind==='bushin'?([0,1,1.7,2.4][selectedTypes.length]||2.4):1;const hp=Math.round(def.hp*partyScale*(awakenedMode?1.7:1)),speed=def.speed*(awakenedMode?1.12:1);Object.assign(this,{...def,hp,maxHp:hp,speed,x:def.kind==='bushin'?700:735,y:510,vx:0,vy:0,r:def.kind==='bushin'?38:def.kind==='dragon'?82:def.kind==='cerberus'?76:def.kind==='troll'?70:64,cd:1.1,attack:0,volley:0,slow:0,freezeStop:0,dead:false,phase:0,inv:0,touchCd:0,bushinState:'idle',bushinTimer:1.0,bushinAir:0,bushinKick:false,bushinLine:null,bushinHitDone:false,bushinStart:null,bushinTarget:null})}
 damage(n){return enemyDamage(n)*(this.kind==='demonking'?1.08:1)}
 hurt(n){if(this.dead)return;if(this.kind==='bushin'&&this.bushinAir>0)return;this.inv=.12;this.hp-=n;burst(this.x,this.y,'#ffd29d',5,150);if(this.hp<=0){this.hp=0;this.dead=true;bossDefeated()}}
 ...
 updateBushin(dt){
  ...
  if(this.bushinState==='idle'){
   ...
   if(this.bushinTimer<=0){const r=Math.random();if(d<165&&r<.48)this.startBushinPunch(target);else if(r<.74)this.startBushinJump(target);else this.startBushinLine(target)}
   return
  }
  ...
  if(this.bushinState==='punchWindup'&&this.bushinTimer<=0){
  ...
  }else if(this.bushinState==='jumpPause'&&this.bushinTimer<=0){this.bushinState='jumpDive';this.bushinTimer=.34;this.bushinKick=true;this.bushinHitDone=false}
   ...
   this.bushinState='lineKick';this.bushinTimer=.16;this.bushinKick=true;const l=this.bushinLine;this.x=l.x2;this.y=l.y2;clampArena(this);
 ...
 finishBushinAction(delay){this.bushinState='idle';this.bushinTimer=delay;this.bushinAir=0;this.bushinKick=false;this.bushinLine=null;this.attack=0}
 ...
 drawBushin(){
  const target=heroes.filter(h=>!h.dead).sort((a,b)=>Math.hypot(a.x-this.x,a.y-this.y)-Math.hypot(b.x-this.x,b.y-this.y))[0];const flip=target?target.x<this.x:false;
  ...
  const key=this.bushinKick?'boss_bushin_kick':'boss_bushin';const size=this.bushinKick?[178,150]:[145,178];if(!drawSprite(sprites[key],this.x,y,size[0],size[1],flip,this.inv>0?.55:1,0)){ctx.fillStyle='#34251d';ctx.beginPath();ctx.arc(this.x,y-20,42,0,Math.PI*2);ctx.fill()}
 }
 draw(){
