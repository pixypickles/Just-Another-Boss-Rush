...
class Boss{
 constructor(def){const partyScale=def.kind==='bushin'?([0,1,1.7,2.4][selectedTypes.length]||2.4):1;const hp=Math.round(def.hp*partyScale*(awakenedMode?1.7:1)),speed=def.speed*(awakenedMode?1.12:1);Object.assign(this,{...def,hp,maxHp:hp,speed,x:def.kind==='bushin'?700:735,y:510,vx:0,vy:0,r:def.kind==='bushin'?38:def.kind==='foxtrio'?48:def.kind==='dragon'?82:def.kind==='cerberus'?76:def.kind==='troll'?70:64,cd:1.1,attack:0,volley:0,slow:0,freezeStop:0,dead:false,phase:0,inv:0,touchCd:0,bushinState:'idle',bushinTimer:1.0,bushinAir:0,bushinKick:false,bushinLine:null,bushinHitDone:false,bushinStart:null,bushinTarget:null,bushinKickDirX:1,bushinCounterTarget:null,bushinCounterTriggered:false,bushinEcho:null,bushinEchoLife:0})}
 ...
 special(target){
  this.phase++;
  if(this.pattern==='foxtrio'){const bases=[0,Math.PI*2/3,Math.PI*4/3],mode=this.phase%4;if(mode===0){this.volley=.8;for(const off of bases){const sx=this.x+Math.cos(off)*105,sy=this.y+Math.sin(off)*75,aim=Math.atan2(target.y-sy,target.x-sx);for(const aoff of [-.17,0,.17]){const a=aim+aoff;shots.push({team:'boss',type:'foxshuriken',x:sx,y:sy,vx:Math.cos(a)*650,vy:Math.sin(a)*650,r:11,life:2.2,damage:this.damage(30)})}}}else if(mode===1){const tx=target.x,ty=target.y;holyFx.push({x:tx,y:ty,type:'iceSpike',r:82,life:.55,max:.55});setTimeout(()=>{if(!running||this.dead)return;for(const h of heroes)if(!h.dead&&Math.hypot(h.x-tx,h.y-ty)<h.r+88){h.hurt(this.damage(34));h.vx*=.08;h.vy*=.08;h.inv=Math.max(h.inv,.22)}burst(tx,ty,'#a8eaff',28,270)},140)}else if(mode===2){const away=norm(this.x-target.x,this.y-target.y),ox=this.x,oy=this.y;this.x+=away.x*100;this.y+=away.y*100;clampArena(this);slashes.push({x:(ox+this.x)/2,y:(oy+this.y)/2,side:away.x<0?-1:1,life:.16,max:.16,ninjaDash:true,length:100})}else{this.inv=1.8;for(let wave=0;wave<3;wave++)setTimeout(()=>{if(!running||this.dead)return;for(let k=0;k<3;k++){const a=wave*.7+k*Math.PI*2/3;shots.push({team:'boss',type:'foxfire',x:this.x,y:this.y,vx:Math.cos(a)*(255+wave*60),vy:Math.sin(a)*(255+wave*60),r:20,life:3,damage:this.damage(70)})}burst(this.x,this.y,'#b9f2ff',20,240)},wave*230)}return}
 ...
 draw(){
  ...
  if(this.kind==='foxtrio'){const target=heroes.filter(h=>!h.dead)[0],flip=target?target.x<this.x:false,t=performance.now()/900;for(let i=0;i<3;i++){const a=t+i*Math.PI*2/3,fx=this.x+Math.cos(a)*105,fy=this.y+Math.sin(a)*72;ctx.save();ctx.globalAlpha=.24;ctx.fillStyle='#000';ctx.beginPath();ctx.ellipse(fx,fy+35,38,12,0,0,Math.PI*2);ctx.fill();drawSprite(sprites.hero_fox,fx,fy,126,142,flip,this.inv>0?.62:1,Math.sin(t*3+i)*3);ctx.restore()}return}
