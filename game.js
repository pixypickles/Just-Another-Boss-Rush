...
class Hero{
 ...
 bloodLaser(dt){if(!boss||boss.dead)return;const aim=norm(boss.x-this.x,boss.y-this.y),sx=this.x+aim.x*28,sy=this.y-24+aim.y*28,maxRange=760;this.facing=aim;this.bloodLaserTime=Math.min(5,this.bloodLaserTime+dt);const width=5+Math.min(9,this.bloodLaserTime*2.4),ex=sx+aim.x*maxRange,ey=sy+aim.y*maxRange,vx=ex-sx,vy=ey-sy,len2=vx*vx+vy*vy,t=clamp(((boss.x-sx)*vx+(boss.y-sy)*vy)/len2,0,1),px=sx+vx*t,py=sy+vy*t,hit=Math.hypot(boss.x-px,boss.y-py)<boss.r+width;bloodBeams.push({x:sx,y:sy,dx:aim.x,dy:aim.y,width,hit});if(hit){const dmg=31*dt;boss.hurt(dmg);this.heal(dmg*.12);if(this.bloodLaserTime>3)burst(boss.x+rnd(-18,18),boss.y+rnd(-18,18),'#b50038',1,55)}}
 ...
 draw(i){
  ...
  const sw=this.type==='dracula'?146:this.type==='monk'?154:this.type==='magicblade'?174:this.type==='ninja'?142:this.type==='highpriest'?132:this.type==='runemage'?128:this.type==='qigong'?138:this.type==='dragonknight'?168:112,sh=this.type==='dracula'?164:this.type==='monk'?174:this.type==='magicblade'?150:this.type==='ninja'?133:this.type==='highpriest'?157:this.type==='runemage'?150:this.type==='qigong'?156:this.type==='dragonknight'?166:126;if(this.hidden){ctx.globalAlpha=.18}if(this.batForm>0)ctx.globalAlpha=.16;if(!drawSprite(img,this.x,this.y+(this.type==='monk'?14:0),sw,sh,flip,this.dead?.32:1,bob)){ctx.save();ctx.translate(this.x,this.y+bob);ctx.fillStyle=this.type==='knight'?'#7488aa':this.type==='mage'?'#304e9c':this.type==='monk'?'#d7ad72':this.type==='magicblade'?'#6d3aa8':this.type==='qigong'?'#c92e2e':this.type==='dragonknight'?'#9f3a2d':this.type==='dracula'?'#221421':'#f2efe5';ctx.beginPath();ctx.arc(0,-14,27,0,Math.PI*2);ctx.fill();ctx.fillRect(-25,5,50,48);ctx.restore();}
...
boss.draw();heroes.forEach((h,i)=>h.draw(i));
 ...
 for(const l of bloodBeams){ctx.save();ctx.globalAlpha=.82;ctx.strokeStyle=l.hit?'#d41446':'#8d1738';ctx.shadowBlur=20;ctx.shadowColor='#ff174f';ctx.lineCap='round';ctx.lineWidth=l.width+7;ctx.beginPath();ctx.moveTo(l.x,l.y);ctx.lineTo(l.x+l.dx*760,l.y+l.dy*760);ctx.stroke();ctx.strokeStyle='#ff8aaa';ctx.lineWidth=Math.max(2,l.width*.34);ctx.stroke();ctx.restore()}
