...
class Hero{
 ...
 draw(i){
  ...
  if(this.type==='dracula'&&this.batForm>0){
   const tm=performance.now()/1000;ctx.save();ctx.translate(this.x,this.y-8+bob);ctx.shadowBlur=16;ctx.shadowColor='#a51e52';
   for(let k=0;k<8;k++){const a=tm*2.6+k*Math.PI*2/8,rr=24+(k%3)*15,bx=Math.cos(a)*rr,by=Math.sin(a*1.15)*rr*.52+Math.sin(tm*7+k)*5,wing=Math.sin(tm*13+k)*7;ctx.save();ctx.translate(bx,by);ctx.rotate(a+.25);ctx.globalAlpha=.72+.18*Math.sin(tm*5+k);ctx.fillStyle=k%2?'#351025':'#4c1534';ctx.strokeStyle='#e05b88';ctx.lineWidth=1.8;ctx.beginPath();ctx.moveTo(-15,1);ctx.quadraticCurveTo(-8,-10-wing,0,-3);ctx.quadraticCurveTo(8,-10+wing,15,1);ctx.quadraticCurveTo(8,8,0,4);ctx.quadraticCurveTo(-8,8,-15,1);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle='#ff668f';ctx.beginPath();ctx.arc(2,-1,1.8,0,Math.PI*2);ctx.fill();ctx.restore()}
   ctx.globalAlpha=.34;ctx.strokeStyle='#d64e7d';ctx.lineWidth=3;ctx.beginPath();ctx.ellipse(0,22,46,17,0,0,Math.PI*2);ctx.stroke();ctx.restore();
  }else if(!drawSprite(img,this.x,this.y+(this.type==='monk'?14:0),sw,sh,flip,this.dead?.32:1,bob)){ctx.save();ctx.translate(this.x,this.y+bob);ctx.fillStyle=this.type==='knight'?'#7488aa':this.type==='mage'?'#304e9c':this.type==='monk'?'#d7ad72':this.type==='magicblade'?'#6d3aa8':this.type==='qigong'?'#c92e2e':this.type==='dragonknight'?'#9f3a2d':this.type==='dracula'?'#221421':'#f2efe5';ctx.beginPath();ctx.arc(0,-14,27,0,Math.PI*2);ctx.fill();ctx.fillRect(-25,5,50,48);ctx.restore();}
