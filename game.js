...
const AWAKEN_UNLOCK_KEY='jabrAwakeningUnlockedV1',MONK_UNLOCK_KEY='jabrMonkUnlockedV1',HIGHPRIEST_UNLOCK_KEY='jabrHighPriestUnlockedV1',DRAGONKNIGHT_UNLOCK_KEY='jabrDragonKnightUnlockedV1';
...
const heroInfo={knight:{name:'ナイト',hp:540,speed:188,ability:'A：剣・大ダメージ　B：円形バリア　C：自己回復・中'},mage:{name:'魔法使い',hp:340,speed:175,ability:'A：2方向ファイアボール　B：2方向フリーズ　C：小竜巻'},healer:{name:'ヒーラー',hp:390,speed:188,ability:'A：誘導ライトボール　B：範囲回復・中　C：反射壁'},monk:{name:'モンク',hp:650,speed:196,ability:'A：連続パンチ　B：気功弾　C：気合い（弾消去・吹き飛ばし・攻撃＆自然回復アップ）'},magicblade:{name:'魔剣士',hp:470,speed:192,ability:'A：魔力斬り（弾消去・吸収）　B：三日月の魔剣波（弾消去・吸収）　C：回転斬り（吸収）　1人D：魔人化'},ninja:{name:'忍者',hp:400,speed:246,ability:'A：自動追尾手裏剣3連発　B：瞬身斬　C：煙玉・反対位置へ瞬移　1人D：分身'},highpriest:{name:'ハイプリースト',hp:410,speed:184,ability:'A：十字の中回復　B：範囲リジェネ　C：広範囲大回復　1人D：神威降臨'},runemage:{name:'ルーンメイジ',hp:420,speed:202,ability:'A：炎ルーン×3　B：氷ルーン×4　C：雷ルーン×5　1人D：ルーンオーバーロード'},qigong:{name:'気功師',hp:460,speed:190,ability:'A：集束気功弾　B：規則拡散気功弾　C：爆芯掌（上限なし・中心ほど高威力）　1人D：気脈解放'},dragonknight:{name:'竜騎士',hp:560,speed:205,ability:'A：長射程ドラゴンスラスト　B：ドラゴンチャージ　C：広範囲ドラゴンスイープ　1人D：三竜の息吹'}};
class Hero{
 constructor(type,x,y){const i=heroInfo[type];Object.assign(this,{type,x,y,vx:0,vy:0,r:24,hp:i.hp,maxHp:i.hp,cdA:0,cdB:0,cdC:0,inv:0,guard:0,tornado:0,drainFx:0,dead:false,facing:{x:1,y:0},attackAnim:0,attackSide:1,soloCd:0,spiritFx:0,healFx:0,powerUp:0,demonMode:0,cloneTime:0,smokeTime:0,hidden:false,divineMode:0,runeOverload:0,regenTime:0,regenRate:0,chargeA:0,chargeB:0,chargeC:0,qigongFocus:0,qigongALock:0,qigongBLock:0,dragonBreath:0,breathTick:0,dragonChargeFx:0})}
 heal(n){if(this.dead)return;this.hp=Math.min(this.maxHp,this.hp+n);burst(this.x,this.y,'#7dffad',10,120)}
 hurt(n,kx=0,ky=0){if(this.dead||this.inv>0)return;if(this.guard>0)n*=.1;if(this.type==='qigong'&&(this.chargeA>0||this.chargeB>0||this.chargeC>0))n*=2;this.hp-=n;this.vx+=kx;this.vy+=ky;this.inv=.3;shake=9;burst(this.x,this.y,'#ff6571',15,220);if(this.hp<=0){this.hp=0;this.dead=true;partyDeaths++;notice(heroInfo[this.type].name+'が倒れた！','#ff8c97')}}
 update(dt,i){this.cdA=Math.max(0,this.cdA-dt);this.cdB=Math.max(0,this.cdB-dt);this.cdC=Math.max(0,this.cdC-dt);this.inv=Math.max(0,this.inv-dt);this.guard=Math.max(0,this.guard-dt);this.tornado=Math.max(0,this.tornado-dt);this.drainFx=Math.max(0,this.drainFx-dt);this.spiritFx=Math.max(0,this.spiritFx-dt);this.healFx=Math.max(0,this.healFx-dt);this.powerUp=Math.max(0,this.powerUp-dt);this.demonMode=Math.max(0,this.demonMode-dt);this.cloneTime=Math.max(0,this.cloneTime-dt);this.divineMode=Math.max(0,this.divineMode-dt);this.runeOverload=Math.max(0,this.runeOverload-dt);this.qigongFocus=Math.max(0,this.qigongFocus-dt);this.qigongALock=Math.max(0,this.qigongALock-dt);this.qigongBLock=Math.max(0,this.qigongBLock-dt);this.dragonBreath=Math.max(0,this.dragonBreath-dt);this.breathTick=Math.max(0,this.breathTick-dt);this.dragonChargeFx=Math.max(0,this.dragonChargeFx-dt);if(this.regenTime>0){this.regenTime=Math.max(0,this.regenTime-dt);this.hp=Math.min(this.maxHp,this.hp+this.regenRate*dt)}this.smokeTime=Math.max(0,this.smokeTime-dt);this.attackAnim=Math.max(0,this.attackAnim-dt);this.soloCd=Math.max(0,this.soloCd-dt);if(awakenedMode){this.cdA=0;this.cdB=0;this.cdC=0;this.soloCd=0}if(this.dead)return;if(this.type==='knight'&&this.guard>0)this.hp=Math.min(this.maxHp,this.hp+8*dt);if(this.type==='monk')this.hp=Math.min(this.maxHp,this.hp+(this.powerUp>0?14:4.5)*dt);
  if(this.type==='dragonknight'&&this.dragonBreath>0&&this.breathTick<=0){this.breathTick=.14;const aim=boss&&!boss.dead?norm(boss.x-this.x,boss.y-this.y):this.facing;this.facing=aim;const base=Math.atan2(aim.y,aim.x);[-.19,0,.19].forEach((off,k)=>{const a=base+off,side=k-1,ox=this.x+Math.cos(a)*44-Math.sin(a)*side*28,oy=this.y-27+Math.sin(a)*44+Math.cos(a)*side*28;shots.push({team:'hero',type:'dragonfire',owner:this,x:ox,y:oy,vx:Math.cos(a)*520,vy:Math.sin(a)*520,r:18,life:.72,damage:16,bulletCut:true,pierce:2});burst(ox,oy,'#ff7a38',5,100)});}
  let mx=0,my=0;
  if(i===heroIndex){mx=(keys.has('KeyD')||keys.has('ArrowRight')?1:0)-(keys.has('KeyA')||keys.has('ArrowLeft')?1:0)+joy.x;my=(keys.has('KeyS')||keys.has('ArrowDown')?1:0)-(keys.has('KeyW')||keys.has('ArrowUp')?1:0)+joy.y;if(this.type==='qigong'){const rate=this.qigongFocus>0?5:1;if(pressed.has('KeyJ')){pressed.delete('KeyJ');if(this.qigongFocus>0&&this.qigongALock<=0){this.qigongALock=.32;this.fireQigongA(3)}}if(pressed.has('KeyK')){pressed.delete('KeyK');if(this.qigongFocus>0&&this.qigongBLock<=0){this.qigongBLock=.72;this.fireQigongB(4)}}if(keys.has('KeyJ')&&this.qigongFocus<=0)this.chargeA=Math.min(3,this.chargeA+dt);if(keys.has('KeyK')&&this.qigongFocus<=0)this.chargeB=Math.min(4,this.chargeB+dt);if(keys.has('KeyL'))this.chargeC+=dt*rate;if(released.has('KeyJ')&&this.chargeA>0){this.fireQigongA(this.chargeA);this.chargeA=0}if(released.has('KeyK')&&this.chargeB>0){this.fireQigongB(this.chargeB);this.chargeB=0}if(released.has('KeyL')&&this.chargeC>0){this.fireQigongC(this.chargeC);this.chargeC=0}}else{if(pressed.has('KeyJ')){pressed.delete('KeyJ');this.a()}if(pressed.has('KeyK')){pressed.delete('KeyK');this.b()}if(pressed.has('KeyL')){pressed.delete('KeyL');this.c()}if(this.type==='runemage'&&this.runeOverload>0){if(keys.has('KeyJ')&&this.cdA<=0)this.a();if(keys.has('KeyK')&&this.cdB<=0)this.b();if(keys.has('KeyL')&&this.cdC<=0)this.c()}}}
  else{const d=Math.hypot(boss.x-this.x,boss.y-this.y),n=norm(boss.x-this.x,boss.y-this.y);if(this.type==='knight'){mx=n.x;my=n.y;if(d<105&&this.cdA<=0)this.a();if(this.cdB<=0)this.b();if(this.hp<this.maxHp*.33&&this.cdC<=0)this.c()}else if(this.type==='mage'){if(d<360){mx=-n.x;my=-n.y}else if(d>470){mx=n.x;my=n.y}else{mx=-n.y*.34;my=n.x*.34}if(this.cdA<=0)this.a();if(this.cdB<=0&&Math.random()<.028)this.b();if((d<155||boss.volley>0)&&this.cdC<=0)this.c()}else if(this.type==='monk'){mx=n.x;my=n.y;if(d<105&&this.cdA<=0)this.a();if(d>=105&&this.cdB<=0)this.b();if((d<125||boss.volley>0)&&this.cdC<=0)this.c()}else if(this.type==='magicblade'){if(d>135){mx=n.x;my=n.y}else{mx=-n.y*.28;my=n.x*.28}if(d<145&&this.cdA<=0)this.a();if(d>=120&&this.cdB<=0)this.b();if((this.hp<this.maxHp*.65||boss.volley>0)&&this.cdC<=0)this.c()}else if(this.type==='ninja'){if(d>260){mx=n.x;my=n.y}else{mx=-n.y*.55;my=n.x*.55}if(this.cdA<=0)this.a();if(d>170&&this.cdB<=0&&Math.random()<.035)this.b();if((boss.volley>0||d<115)&&this.cdC<=0)this.c()}else if(this.type==='runemage'){if(d<260){mx=-n.x*.55;my=-n.y*.55}else if(d>470){mx=n.x*.35;my=n.y*.35}else{mx=-n.y*.42;my=n.x*.42}if(this.cdA<=0)this.a();if(this.cdB<=0&&Math.random()<.045)this.b();if(this.cdC<=0&&Math.random()<.085)this.c()}else if(this.type==='dragonknight'){if(d>235){mx=n.x;my=n.y}else{mx=-n.y*.25;my=n.x*.25}if(d<340&&this.cdA<=0)this.a();if(d>180&&this.cdB<=0&&Math.random()<.025)this.b();if((minions.length>2||d<155)&&this.cdC<=0)this.c()}else if(this.type==='qigong'){if(d<280){mx=-n.x*.45;my=-n.y*.45}else if(d>500){mx=n.x*.35;my=n.y*.35}else{mx=-n.y*.3;my=n.x*.3}if(this.cdA<=0)this.fireQigongA(2.2);if(this.cdB<=0&&Math.random()<.035)this.fireQigongB(3.2)}else if(this.type==='highpriest'){const low=heroes.filter(h=>!h.dead).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(low&&low.hp/low.maxHp<.7){mx=low.x-this.x;my=low.y-this.y;if(this.cdA<=0)this.a();if(this.cdB<=0)this.b();if(low.hp/low.maxHp<.38&&this.cdC<=0)this.c()}else{if(d<430){mx=-n.x*.55;my=-n.y*.55}else{mx=n.x*.25;my=n.y*.25}}}else{const low=heroes.filter(h=>!h.dead).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(low&&low.hp/low.maxHp<.82){mx=low.x-this.x;my=low.y-this.y;if(Math.hypot(mx,my)<245&&this.cdB<=0)this.b()}else{if(d<390){mx=-n.x*.7;my=-n.y*.7}else if(d>520){mx=n.x*.55;my=n.y*.55}else{mx=-n.y*.22;my=n.x*.22}if(this.cdA<=0)this.a();if(this.cdC<=0&&boss.volley>0)this.c()}}}
  const has=Math.abs(mx)+Math.abs(my)>.05,n=norm(mx,my),sp=heroInfo[this.type].speed;this.vx+=(has?n.x*sp-this.vx:-this.vx)*Math.min(1,dt*13);this.vy+=(has?n.y*sp-this.vy:-this.vy)*Math.min(1,dt*13);if(has)this.facing=n;this.x+=this.vx*dt;this.y+=this.vy*dt;clampArena(this);
  if(this.tornado>0){
  ...
  fireQigongA(charge){if(this.cdA>0)return;const t=clamp(charge/3,0,1),aim=boss&&!boss.dead?norm(boss.x-this.x,boss.y-this.y):this.facing,hand=this.qigongHandPoint();this.facing=aim;this.cdA=.45;shots.push({team:'hero',type:'qigong',owner:this,x:hand.x,y:hand.y,vx:aim.x*(620+180*t),vy:aim.y*(620+180*t),r:10+18*t,life:2.1,damage:38+122*t,bulletCut:true});burst(hand.x,hand.y,'#ffcf6a',14+18*t,180+180*t);notice('集束気功弾！','#ffe09b',420)}
  fireQigongB(charge){if(this.cdB>0)return;const t=clamp(charge/4,0,1),aim=boss&&!boss.dead?norm(boss.x-this.x,boss.y-this.y):this.facing,base=Math.atan2(aim.y,aim.x),count=3+2*Math.floor(t*3),rows=1+Math.floor(t*2),spread=.18+.42*t,hand=this.qigongHandPoint(),pellets=count*rows,totalDamage=150+210*t,shotDamage=totalDamage/pellets;this.facing=aim;this.cdB=.75;for(let row=0;row<rows;row++)for(let k=0;k<count;k++){const u=count===1?0:k/(count-1)-.5,a=base+u*spread,dir={x:Math.cos(a),y:Math.sin(a)},back=row*8;shots.push({team:'hero',type:'qigong',owner:this,x:hand.x-dir.x*back,y:hand.y-dir.y*back,vx:dir.x*(610+row*28),vy:dir.y*(610+row*28),r:9+4*t,life:2,damage:shotDamage,bulletCut:true})}burst(hand.x,hand.y,'#ffd36d',22+18*t,260);notice('拡散気功弾！','#ffe29c',500)}
  fireQigongC(charge){const aim=boss&&!boss.dead?norm(boss.x-this.x,boss.y-this.y):this.facing,hand=this.qigongHandPoint(),cx=hand.x,cy=hand.y,scale=Math.max(.04,charge/30),radius=95+Math.min(260,charge*5.5),ring=d=>d<=radius*.2?1:d<=radius*.4?.8:d<=radius*.6?.6:d<=radius*.8?.4:.2,base=80+(boss&&!boss.dead?boss.maxHp*1.08:2200)*scale*scale;this.facing=aim;this.cdC=.9;for(let i=shots.length-1;i>=0;i--){const q=shots[i];if(q.team==='boss'&&Math.hypot(q.x-cx,q.y-cy)<radius+q.r){burst(q.x,q.y,'#fff0a8',7,120);shots.splice(i,1)}}if(!boss.dead){const d=Math.hypot(boss.x-cx,boss.y-cy);if(d<radius+boss.r){boss.hurt(base*ring(Math.max(0,d-boss.r)));shake=22}}for(const m of minions){const d=Math.hypot(m.x-cx,m.y-cy);if(d<radius+m.r)m.hp-=Math.min(2500,base*ring(Math.max(0,d-m.r)))}holyFx.push({x:cx,y:cy,type:'circle',r:radius,life:.75,max:.75,big:true});burst(cx,cy,'#fff0a0',60,620);notice(`爆芯掌！ ${charge.toFixed(1)}秒チャージ`,'#fff1a6',900)}
 dragonThrust(){const dir=boss&&!boss.dead?(boss.x<this.x?-1:1):(this.facing.x<0?-1:1),range=285,width=48;this.facing={x:dir,y:0};this.attackAnim=.30;this.attackSide=dir;slashes.push({x:this.x+dir*range*.48,y:this.y-12,side:dir,life:.24,max:.24,dragonThrust:true,length:range});for(let i=shots.length-1;i>=0;i--){const q=shots[i];if(q.team==='boss'&&Math.abs(q.y-(this.y-12))<width+q.r&&q.x>Math.min(this.x,this.x+dir*range)-q.r&&q.x<Math.max(this.x,this.x+dir*range)+q.r){burst(q.x,q.y,'#ffb06d',7,130);shots.splice(i,1)}}for(const m of minions)if(Math.abs(m.y-(this.y-12))<width+m.r&&m.x>Math.min(this.x,this.x+dir*range)-m.r&&m.x<Math.max(this.x,this.x+dir*range)+m.r){m.hp-=125;m.vx+=dir*300}if(!boss.dead&&Math.abs(boss.y-(this.y-12))<width+boss.r&&boss.x>Math.min(this.x,this.x+dir*range)-boss.r&&boss.x<Math.max(this.x,this.x+dir*range)+boss.r){boss.hurt(108);boss.vx+=dir*230;shake=13}burst(this.x+dir*range,this.y-12,'#ffcf91',26,330);notice('ドラゴンスラスト！','#ffd09a',500)}
 dragonCharge(){const aim=boss&&!boss.dead?norm(boss.x-this.x,boss.y-this.y):this.facing,startX=this.x,startY=this.y,dist=410,endX=this.x+aim.x*dist,endY=this.y+aim.y*dist;this.facing=aim;this.inv=.58;this.dragonChargeFx=.42;for(let i=shots.length-1;i>=0;i--){const q=shots[i];if(q.team!=='boss')continue;const vx=endX-startX,vy=endY-startY,len2=vx*vx+vy*vy,t=clamp(((q.x-startX)*vx+(q.y-startY)*vy)/len2,0,1),px=startX+vx*t,py=startY+vy*t;if(Math.hypot(q.x-px,q.y-py)<52+q.r){burst(q.x,q.y,'#ff8c45',8,150);shots.splice(i,1)}}for(const m of minions){const vx=endX-startX,vy=endY-startY,len2=vx*vx+vy*vy,t=clamp(((m.x-startX)*vx+(m.y-startY)*vy)/len2,0,1),px=startX+vx*t,py=startY+vy*t;if(Math.hypot(m.x-px,m.y-py)<58+m.r){m.hp-=135;m.vx+=aim.x*500;m.vy+=aim.y*500}}if(!boss.dead){const vx=endX-startX,vy=endY-startY,len2=vx*vx+vy*vy,t=clamp(((boss.x-startX)*vx+(boss.y-startY)*vy)/len2,0,1),px=startX+vx*t,py=startY+vy*t;if(Math.hypot(boss.x-px,boss.y-py)<60+boss.r){boss.hurt(112);boss.vx+=aim.x*390;boss.vy+=aim.y*390;shake=15}}this.x=endX;this.y=endY;clampArena(this);this.vx=aim.x*520;this.vy=aim.y*520;burst(startX,startY,'#ff6d36',24,300);burst(this.x,this.y,'#ffc26d',34,420);notice('ドラゴンチャージ！','#ffb36b',650)}
 ...
 soloSkill(){
  ...
  if(this.type==='knight'){
   ...
   this.soloCd=14;this.qigongFocus=10;this.inv=.45;burst(this.x,this.y,'#ffcf5c',58,480);notice('気脈解放！ 10秒間 A・B即最大／Cチャージ5倍！','#ffe39a',1200)
 ...
 draw(i){
  ...
  const sw=this.type==='monk'?154:this.type==='magicblade'?174:this.type==='ninja'?142:this.type==='highpriest'?132:this.type==='runemage'?128:this.type==='qigong'?138:this.type==='dragonknight'?168:112,sh=this.type==='monk'?174:this.type==='magicblade'?150:this.type==='ninja'?133:this.type==='highpriest'?157:this.type==='runemage'?150:this.type==='qigong'?156:this.type==='dragonknight'?166:126;if(this.hidden){ctx.globalAlpha=.18}if(!drawSprite(img,this.x,this.y+(this.type==='monk'?14:0),sw,sh,flip,this.dead?.32:1,bob)){ctx.save();ctx.translate(this.x,this.y+bob);ctx.fillStyle=this.type==='knight'?'#7488aa':this.type==='mage'?'#304e9c':this.type==='monk'?'#d7ad72':this.type==='magicblade'?'#6d3aa8':this.type==='qigong'?'#c92e2e':this.type==='dragonknight'?'#9f3a2d':'#f2efe5';ctx.beginPath();ctx.arc(0,-14,27,0,Math.PI*2);ctx.fill();ctx.fillRect(-25,5,50,48);ctx.restore();}
  ...
  if(this.type==='qigong'){const total=Math.max(this.chargeA,this.chargeB,this.chargeC),hand=this.qigongHandPoint(),cx=hand.x,cy=hand.y;if(total>0){const c=this.chargeC>0?this.chargeC:Math.max(this.chargeA,this.chargeB),radius=this.chargeC>0?95+Math.min(260,c*5.5):26+Math.min(42,c*12);ctx.save();ctx.globalAlpha=.72;ctx.strokeStyle='#ffe29a';ctx.shadowBlur=18;ctx.shadowColor='#ffbd3e';ctx.lineWidth=3;for(let k=1;k<=5;k++){ctx.globalAlpha=.16+k*.08;ctx.beginPath();ctx.arc(cx,cy,radius*k/5,0,Math.PI*2);ctx.stroke()}ctx.fillStyle='#fff1b0';ctx.globalAlpha=.7;ctx.beginPath();ctx.arc(cx,cy,8+Math.min(22,c*2),0,Math.PI*2);ctx.fill();ctx.restore()}if(this.qigongFocus>0){ctx.save();ctx.globalAlpha=.48;ctx.strokeStyle='#ffcf5c';ctx.lineWidth=6;ctx.shadowBlur=24;ctx.shadowColor='#ff9f2f';ctx.beginPath();ctx.arc(this.x,this.y,62,0,Math.PI*2);ctx.stroke();ctx.restore()}}
  if(this.type==='dragonknight'&&this.dragonBreath>0){const aim=boss&&!boss.dead?norm(boss.x-this.x,boss.y-this.y):this.facing,ang=Math.atan2(aim.y,aim.x);ctx.save();ctx.translate(this.x,this.y-38);ctx.rotate(ang);for(const side of [-1,1]){ctx.save();ctx.translate(5,side*37);ctx.globalAlpha=.38+.12*Math.sin(performance.now()/100+side);ctx.fillStyle='#a12f22';ctx.strokeStyle='#ffb071';ctx.lineWidth=3;ctx.shadowBlur=18;ctx.shadowColor='#ff6a32';ctx.beginPath();ctx.moveTo(4,0);ctx.lineTo(34,-16);ctx.lineTo(58,0);ctx.lineTo(34,16);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle='#ffd36e';ctx.beginPath();ctx.arc(39,-5,3,0,Math.PI*2);ctx.fill();ctx.restore()}ctx.restore()}
...
function bossDefeated(){awakeningSoloCarry=null;if(awakenedMode){const h=heroes.find(x=>!x.dead);if(h&&((h.type==='magicblade'&&h.demonMode>0)||(h.type==='ninja'&&h.cloneTime>0)||(h.type==='runemage'&&h.runeOverload>0)||(h.type==='highpriest'&&h.divineMode>0)||(h.type==='qigong'&&h.qigongFocus>0)||(h.type==='dragonknight'&&h.dragonBreath>0)))awakeningSoloCarry={type:h.type}}transition=2.2;notice('BOSS DEFEATED!','#fff08a',1500);for(const h of heroes)if(!h.dead)h.heal(h.maxHp*.24)}
...
function draw(){
 ...
 ctx.save();ctx.translate(w.owner.x,w.owner.y);
 const pulse=1+Math.sin(performance.now()/110)*.03;ctx.scale(pulse,pulse);
 ctx.globalAlpha=.20;ctx.fillStyle='#eadcff';ctx.beginPath();ctx.arc(0,0,w.r,0,Math.PI*2);ctx.fill();
 ctx.globalAlpha=.82;ctx.strokeStyle='#f3e8ff';ctx.lineWidth=10;ctx.beginPath();ctx.arc(0,0,w.r,0,Math.PI*2);ctx.stroke();
 ctx.globalAlpha=.42;ctx.strokeStyle='#b78cff';ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,0,w.r-14,0,Math.PI*2);ctx.stroke();
 ctx.restore()
}
...
partyChoices.forEach(b=>b.addEventListener('click',()=>{
 if(b.classList.contains('unlockLocked')){document.getElementById('setupStatus').textContent=b.dataset.hero==='highpriest'?'ハイプリーストは、どのモードでも全員生存でクリアすると解放されます':b.dataset.hero==='dragonknight'?'竜騎士は通常モードを1回クリアすると解放されます':'モンクは通常モードを1回クリアすると解放されます';return}
