...
function refreshTrainingPartyUi(){
 ...
 trainingPlay.disabled=!unlocked||count<1||count>3;trainingLimitPlay.disabled=!unlocked||count<1||count>3;trainingMolePlay.disabled=!unlocked||!isMoleUnlocked()||count!==1||!MOLE_ALLOWED.includes(trainingSelectedTypes[0]);trainingShootingPlay.disabled=!unlocked||count!==1||!SHOOTING_ALLOWED.includes(trainingSelectedTypes[0]);trainingPrecisionPlay.disabled=!unlocked||count!==1||!SHOOTING_ALLOWED.includes(trainingSelectedTypes[0]);trainingSoccerPlay.disabled=!unlocked||count<1||count>3||trainingSelectedTypes.includes('mimic');
}
...
trainingSoccerPlay.addEventListener('click',e=>{if(trainingSelectedTypes.includes('mimic')){trainingPartySummary.textContent='模倣術師はボスセット専用のため、神鉄球サッカーでは選択できません';return}startTraining(e,'soccer')});
