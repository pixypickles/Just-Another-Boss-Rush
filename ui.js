...
function shootingRankHtml(){const book=readShootingScoreBook();return `<b>射撃訓練 キャラ別 TOP 3</b>`+SHOOTING_ALLOWED.map(type=>{const ranks=book[type]||[];return `<div class="rankSection"><div class="rankTitle">${heroInfo[type]?.name||type}</div>${ranks.length?ranks.map((r,i)=>`<div class="rankRow"><span>${i+1}位</span><span></span><strong>${r.score}点</strong></div>`).join(''):'<div>記録はまだありません。</div>'}</div>`}).join('')}
function refreshTrainingMenu(){const unlocked=isTrainingUnlocked();trainingMenuButton.disabled=!unlocked;trainingMenuButton.textContent=unlocked?'修行場':'修行場（通常クリアで解放）';trainingPlay.innerHTML='<strong>100体撃破タイムアタック</strong><span>100体を倒すまでの最速タイムを競う</span>';trainingLimitPlay.innerHTML='<strong>60秒・制限時間撃破</strong><span>覚醒無双で60秒間の撃破数を競う</span>';trainingMolePlay.innerHTML=`<strong>モグラ叩き${isMoleUnlocked()?'':'（100体撃破クリアで解放）'}</strong><span>移動方向とABCDを組み合わせて斜めにも叩ける・近接4職の1人専用</span>`;trainingShootingPlay.innerHTML='<strong>射撃訓練</strong><span>魔法使い・忍者・アークメイジ・白狐忍の1人専用／60秒</span>';trainingRank.innerHTML=trainingRankHtml('100体撃破',readTrainingRankBook())+trainingRankHtml('60秒撃破',readTrainingScoreBook(),true)+moleRankHtml()+shootingRankHtml();buildTrainingPartyChoices()}
refreshTrainingMenu();
...
function startTraining(e,challenge='time'){
 ...
 startingGame=true;trainingPlay.disabled=true;trainingLimitPlay.disabled=true;trainingMolePlay.disabled=true;trainingShootingPlay.disabled=true;document.getElementById('loadStatus').textContent=challenge==='mole'?'モグラ叩きを準備しています…':challenge==='shooting'?'射撃訓練を準備しています…':`${need}人部門の${challenge==='limit'?'60秒撃破':'100体撃破'}を準備しています…`;
 const launch=()=>{try{trainingPanel.hidden=true;mainActions.hidden=false;partyDeaths=0;awakeningSoloCarry=null;trainingMode=true;bushinMode=foxMode=hardMode=false;difficultyMode='normal';enemyHpMultiplier=enemyAttackMultiplier=1;difficultyLabel='';awakenedMode=trainingChallenge==='limit';bossIndex=0;transition=0;if(trainingChallenge==='mole')setupMoleBattle();else if(trainingChallenge==='shooting')setupShootingBattle();else setupTrainingBattle();running=true;last=performance.now();ui.notice.style.opacity=0;ui.start.style.display='none';startingGame=false;refreshTrainingMenu()}catch(err){running=false;startingGame=false;ui.start.style.display='grid';refreshTrainingMenu();document.getElementById('loadStatus').textContent='修行場開始エラー: '+(err?.message||err);console.error(err)}};
...
trainingShootingPlay.addEventListener('click',e=>{if(trainingSelectedTypes.length!==1||!SHOOTING_ALLOWED.includes(trainingSelectedTypes[0])){trainingPartySummary.textContent='射撃訓練は魔法使い・忍者・アークメイジ・白狐忍から1人選んでください';return}startTraining(e,'shooting')});
