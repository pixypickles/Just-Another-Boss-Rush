...
function refreshTrainingMenu(){const unlocked=isTrainingUnlocked();trainingMenuButton.disabled=!unlocked;trainingMenuButton.textContent=unlocked?'修行場':'修行場（通常クリアで解放）';trainingPlay.innerHTML='<strong>100体撃破タイムアタック</strong><span>100体を倒すまでの最速タイムを競う</span>';trainingLimitPlay.innerHTML='<strong>60秒・制限時間撃破</strong><span>覚醒無双で60秒間の撃破数を競う</span>';trainingMolePlay.innerHTML=`<strong>モグラ叩き${isMoleUnlocked()?'':'（100体撃破クリアで解放）'}</strong><span>移動方向とABCDを組み合わせて斜めにも叩ける・近接4職の1人専用</span>`;trainingShootingPlay.innerHTML='<strong>迎撃訓練</strong><span>横から迫る飛行モンスターを連射で迎撃／射撃4職の1人専用・60秒</span>';trainingPrecisionPlay.innerHTML='<strong>射撃訓練</strong><span>上から流れる的を狙い、中心命中ほど高得点／射撃4職の1人専用・60秒</span>';trainingRank.innerHTML=trainingRankHtml('100体撃破',readTrainingRankBook())+trainingRankHtml('60秒撃破',readTrainingScoreBook(),true)+moleRankHtml()+shootingRankHtml()+precisionRankHtml();buildTrainingPartyChoices()}
refreshTrainingMenu();
...
function startTraining(e,challenge='time'){
 ...
 startingGame=true;trainingPlay.disabled=true;trainingLimitPlay.disabled=true;trainingMolePlay.disabled=true;trainingShootingPlay.disabled=true;trainingPrecisionPlay.disabled=true;document.getElementById('loadStatus').textContent=challenge==='mole'?'モグラ叩きを準備しています…':challenge==='shooting'?'迎撃訓練を準備しています…':challenge==='precision'?'射撃訓練を準備しています…':`${need}人部門の${challenge==='limit'?'60秒撃破':'100体撃破'}を準備しています…`;
 const launch=()=>{try{trainingPanel.hidden=true;mainActions.hidden=false;partyDeaths=0;awakeningSoloCarry=null;trainingMode=true;bushinMode=foxMode=hardMode=false;difficultyMode='normal';enemyHpMultiplier=enemyAttackMultiplier=1;difficultyLabel='';awakenedMode=trainingChallenge==='limit';bossIndex=0;transition=0;if(trainingChallenge==='mole')setupMoleBattle();else if(trainingChallenge==='shooting')setupShootingBattle();else if(trainingChallenge==='precision')setupPrecisionShootingBattle();else setupTrainingBattle();running=true;last=performance.now();ui.notice.style.opacity=0;ui.start.style.display='none';startingGame=false;refreshTrainingMenu()}catch(err){running=false;startingGame=false;ui.start.style.display='grid';refreshTrainingMenu();document.getElementById('loadStatus').textContent='修行場開始エラー: '+(err?.message||err);console.error(err)}};
